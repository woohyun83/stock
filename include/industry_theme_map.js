...
async function industry_build(clientSession) {
  try {
    ...
    try{
      await clientSession.set('industry_map', JSON.stringify(map));
...
async function theme_build(clientSession) {
  try {
    ...
    try{
      await clientSession.set('theme_map', JSON.stringify(map));
